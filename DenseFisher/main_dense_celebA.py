...
if opt.experiment is None:
    opt.experiment = 'celeba_gdense_alt_complete'
...
#netG = densegan.DC_netG2_64(opt.imageSize, nz, ngpu)
#netG = densegan.GenDenseNet_64(growthRate=12, depth=13, increase=1, nz = 100, verbose=0,  bottleneck=True)
#netG = densegan.Dense_netG3_64(nz, ngpu)
netG = densegan.GenDenseNet_64_alt(growthRate=12, depth=13, increase=1, nz = 100, verbose=0)
...
'''    
netD = densegan.DC_netD2_64(opt.imageSize, ngpu)
...
for epoch in range(opt.niter):
    ...
    while i < len(dataloader):
        ...
        if opt.hiDiterStart and (gen_iterations < 25 or gen_iterations % 300 == 0):
            Diters = 100
        else:
            Diters = opt.Diters
        j = 0
